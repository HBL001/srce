/*!
 * \file ft800_if.h
 * \brief FT800 Interface implementation
 */
 ...
 {
     uint32_t result = macro_bits;
 
     result |= ((uint32_t)macro_id & 0x01);
 ...
 {
     uint32_t result = color_alpha_bits;
 
     result |= ((uint32_t)alpha);
 ...
 {
     uint32_t result = vertex2f_bits;
 
     result |= ((uint32_t)(x & 0x7FFF)) << 15;
     result |= ((uint32_t)(y & 0x7FFF));
 ...
 {
     uint32_t result = line_width_bits;
 
     result |= (uint32_t)(width & 0xFFF);
 ...
 {
     uint32_t result = point_size_bits;
 
     result |= (uint32_t)(size & 0xFFF);
 ...
 {
     uint32_t result = bitmap_handle_bits;
